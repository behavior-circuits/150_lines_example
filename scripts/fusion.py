...
class Fusion:
	def __init__(self):
                ...
                parameters     =  np.array([[-0.5,-1,1,-1],[0.3,0,1,1],[0,0,1,1],[0.8,0,1,1],[-0.5,-1,1,-1]])
                for i in range(5):
        ...
        def generate_behavior(self,msg,args):
                if msg.x != 0 and msg.y != 0 :
                    self.behaviors[args[0]] =  AND(sigmoid(msg.x**args[2],args[1],0),sigmoid(msg.y**args[4],args[3]))
                else:
	...
	def fusion(self):
		...
		cmd_vel = Twist()
		cmd_vel.linear.x = 1
                cmd_vel.angular.z = PREVAIL(OR(INVOKE(self.behaviors[2],self.behaviors[0]),self.behaviors[3]),self.behaviors[4])
